...
部署工厂合约
hh deploy --tags v2_factory --network wannsee
查看 init_hash 复制到路由合约
hh run scripts/index.js --network wannsee
hh run scripts/index.js --network localhost
hh run scripts/index.js --network wannsee_mainnet
部署其他合约
hh deploy --tags v2_swap --network wannsee
hh deploy --tags v2_swap --network wannsee_mainnet

修改 sdk 对应地址 - [FACTORY, INIT_CODE_HASH, WMXC]
factory_address src -> constant.ts FACTORY_ADDRESS
INIT_CODE_PAIR_HASH src -> constant.ts INIT_CODE_HASH
chain_id src -> constant.ts ChainId -> add a deploy network
WMXC address src -> entities -> token.ts
SOLIDITY_TYPE_MAXIMA -> add a network

change fontside - [ROUTER, multcall]
router2_address src -> constant -> index -> ROUTER_ADDRESS
chain_id src -> connectors -> index -> supportedChainIds
